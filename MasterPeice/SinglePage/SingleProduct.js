document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        document.querySelector('#prodetails h3').innerText = `$${data.price}`; // Update product price
...
async function addToCart() {
    ...
    try {
        ...
        console.log(await response.text()); // Log the entire response as text

        if (!response.ok) {
            throw new Error('Network response was not ok');
        }

        const clonedResponse = response.clone(); // Clone the response before reading JSON
